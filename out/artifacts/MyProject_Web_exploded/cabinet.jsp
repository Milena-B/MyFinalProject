...
<main>
    ...
    <div class="main">
        <c:if test="${requestScope.userOrders.size() == 0}">
            <h2 align="center"><fmt:message key="orders"/></h2>
        </c:if>
        <c:if test="${requestScope.userOrders.size() != 0}">
        <table border="2" align="center">
            <tr>
                <th><fmt:message key="cabinet.order.id"/></th>
                <th><fmt:message key="cabinet.order.status"/></th>
                <th><fmt:message key="cabinet.order.address"/></th>
                <th><fmt:message key="cabinet.order.card"/></th>
                <th><fmt:message key="cabinet.order.amount"/></th>
                <th><fmt:message key="cabinet.order.price"/></th>
                <th><fmt:message key="cabinet.order.name"/></th>
            </tr>
            ...
            <c:forEach items="${userOrders}" var="userOrder">
                <tr>
                    ...
                    <td>${userOrder.getCardNumber()}</td>
                    <td>${userOrder.getAmount()}</td>
                    <td>${userOrder.getPrice()}</td>
                    <td>${userOrder.getProductName()}</td>
                </tr>
            </c:forEach>
        </table>
    </div>
        ...
        <div class="col-sm-12">
            <div class="pagination font-alt">
                <c:if test="${requestScope.currentPage != 1}">
                    <a href="cabinet?page=${requestScope.currentPage - 1}">
                ...
                <c:forEach begin="1" end="${requestScope.numberOfPages}" var="i">
                    <c:choose>
                        ...
                        <c:otherwise>
                            <td><a href="cabinet?page=${i}">${i}</a></td>
                        </c:otherwise>
                    </c:choose>
                </c:forEach>
